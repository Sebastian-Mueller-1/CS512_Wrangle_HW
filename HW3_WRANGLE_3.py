...
# convert JSON to CSV
csv25 = pd.read_json('mag_2.5+.json', encoding="utf8")
csv25.to_csv('mag_2.5+.csv',index=False)

csv45 = pd.read_json('mag_2.5+.json', encoding="utf8")
csv45.to_csv('mag_4.5+.csv',index = False)

csvsig = pd.read_json('mag_sig+.json', encoding="utf8")
csvsig.to_csv('mag_sig+.csv',index= False)


# manually prepend outer object for each JSON for correct formatting
prepend = '{\n"2.5+_earthquake_records":'
with open("mag_2.5+.json", 'r+', encoding='utf8') as temp: 
    temp_data = temp.read()
    temp.seek(0,0)
    temp.write(prepend+'\n'+temp_data)
...
with open("mag_4.5+.json", 'r+', encoding='utf8') as temp: 
    temp_data = temp.read()
    temp.seek(0,0)
    temp.write(prepend+'\n'+temp_data)
    temp.seek(0,2) # send pointer to end of file
...
with open("mag_sig+.json", 'r+',encoding='utf8') as temp: 
    temp_data = temp.read()
    temp.seek(0,0)
    temp.write(prepend+'\n'+temp_data)
    temp.seek(0,2) # send pointer to end of file
